...
import { MOCK_PROCESSES } from '../../data/systemData';
...
const SystemMonitor = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'transparent', color: 'var(--os-theme-text)', fontFamily: "'Segoe UI', sans-serif" }}>

      {/* Sidebar / Tabs Header */}
      <div style={{ display: 'flex', background: 'var(--os-theme-card-bg)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      {/* Footer Status */}
      <div style={{ padding: '5px 10px', background: 'var(--os-theme-accent)', color: '#fff', fontSize: '11px', display: 'flex', justifyContent: 'space-between' }}>
...
const GraphWidget = ({ title, data, color, value, sub }) => {
  ...
  return (
    <div style={{ background: 'var(--os-theme-card-bg)', padding: '15px', borderRadius: '8px', borderLeft: `4px solid ${color}` }}>
...
const ProcessesTab = ({ theme }) => (
  <div style={{ height: '100%', overflowY: 'auto' }}>
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', color: theme?.text }}>
      <thead>
        <tr style={{ borderBottom: `1px solid ${theme?.accent}30`, textAlign: 'left' }}>
          <th style={{ padding: '8px', color: theme?.accent }}>Name</th>
          <th style={{ padding: '8px', color: theme?.accent }}>CPU %</th>
          <th style={{ padding: '8px', color: theme?.accent }}>Mem (MB)</th>
          <th style={{ padding: '8px', color: theme?.accent }}>User</th>
        </tr>
      </thead>
      <tbody>
        {MOCK_PROCESSES.map((proc, i) => (
          <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <td style={{ padding: '8px' }}>{proc.mem}</td>
            <td style={{ padding: '8px', color: '#888' }}>{proc.user}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
const InfoItem = ({ label, value }) => (
  <div style={{ background: 'var(--os-theme-card-bg)', padding: '15px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)' }}>
...
const TabButton = ({ active, onClick, icon, label }) => (
  <div
    ...
    style={{
      ...
      borderBottom: active ? '2px solid var(--os-theme-accent)' : '2px solid transparent',
